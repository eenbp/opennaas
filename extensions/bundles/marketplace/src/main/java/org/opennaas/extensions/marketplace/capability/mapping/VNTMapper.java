 ...
 ///// the next method is used to choose the next unmapped vnode  
   ...
   ///// the next method is used to sort the possible real nodes based on the connections number with the selected real nodes.
    ...
    //// the next method is used to match the capacity and location of a virtual node
     ...
     public ArrayList matchVirtualNodeDynamicAttributes(VNTRequest v, int vid, InPNetwork net)
        {
            ArrayList res = new ArrayList();
            for (int i = 0; i < (int)net.nodes.size(); i++)
            {
                if ((net.nodes.get(i).capacity >= v.vnodes.get(vid).capacity) && (Math.abs(net.nodes.get(i).location.row - v.vnodes.get(vid).location.row) <= v.vnodes.get(vid).distance) && (Math.abs(net.nodes.get(i).location.cell - v.vnodes.get(vid).location.cell) <= v.vnodes.get(vid).distance))
                    {
                        res.add(i);
                    }                  
            }
       ...
       ///// the next method is used to match the capacity and location of all virtual nodes
        public int matchVirtualNetworkStaticAndDynamicAttributes(VNTRequest v,InPNetwork net,ArrayList<ArrayList > res )
         {
                      int t=1;	
			...
			for(int i=0;(i<(int)v.vnodes.size())&&(t==1);i++){
			   ArrayList s;
                	   s=matchVirtualNodeDynamicAttributes(v,i,net);
               		   if(s.size()==0) t=-1;
                        ...
                        //// now try to map only the vnodes to the candidate pnodes ///
                        /*
                      if (t == 1)
                        {  
                          
                          
                          for (int e = 0; e < v.vlinks.size(); e++)
                             v.vlinks.get(e).id = -1;                          
                          
                          MappingResult tempRes = new MappingResult();
                          int mappingNodesRes = VNTMapping(v, net, 0, res, 3, tempRes);
                          if (mappingNodesRes != 1)
                             t = 0; 
